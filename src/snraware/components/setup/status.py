...
def benchmark_forward(
    fn, *inputs, min_run_time=10, desc="", verbose=True, amp=False, amp_dtype=torch.float16, **kwinputs
):
    """Use Pytorch Benchmark on the forward pass of an arbitrary function."""
    if verbose:
        print(f"{Fore.CYAN}{desc} - Forward pass{Style.RESET_ALL}")
...
def benchmark_backward(
    fn, *inputs, grad=None, min_run_time=10, desc="", verbose=True, amp=False, amp_dtype=torch.float16, **kwinputs
):
...
def benchmark_all(
    fn, *inputs, grad=None, min_run_time=10, desc="", verbose=True, amp=False, amp_dtype=torch.bfloat16, **kwinputs
):
    """Use Pytorch Benchmark on the forward+backward pass of an arbitrary function."""
    return (
        benchmark_forward(
            fn, *inputs, min_run_time=min_run_time, desc=desc, verbose=verbose, amp=amp, amp_dtype=amp_dtype, **kwinputs
        ),
        benchmark_backward(
            fn,
            *inputs,
...
def benchmark_memory(fn, *inputs, desc="", amp=False, amp_dtype=torch.bfloat16, verbose=True, **kwinputs):
    """Measure the inference gpu useage."""
